...
namespace sistemanalizisunumotomasyon
{
    public partial class depoguncelle : Form
    {
        ...
        {
            db.connection.Open();
            string query_Ara = "Select * from depoyonetimi where depo_yonetimi_id=@depo_yonetimi_id";//yonetici_id sine göre veri geliyor.
            MySqlCommand cmd = new MySqlCommand(query_Ara, db.connection);
            cmd.Parameters.AddWithValue("@depo_yonetimi_id", txt_kul_id.Text);
            ...
            {//formdaki textboxlara, datareader ile gelen verileri aktardık.
                lbl_kul_id.Text = dr["depo_yonetimi_id"].ToString();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string query_Güncelle = "Update depoyonetimi set isim_soyisim=@txt_kul_adısoyadı,sifre=@txt_sifre,maas=@txt_maas,tc_no=@txt_tcno,cep_no=@txt_telno where  depo_yonetimi_id=@depo_yonetimi_id";
            MySqlCommand cmd = new MySqlCommand(query_Güncelle, db.connection);
            cmd.Parameters.AddWithValue("@depo_yonetimi_id", Convert.ToInt32(txt_kul_id.Text.ToString()));
